...
"""
def author():                                                                                             
        return 'yzheng420' # replace gburdell3 with your Georgia Tech username.                                                                                             
                                                                                              
def gtid():                                                                                               
    return 903547918 # replace with your GT ID number      
...
#############################################
    ...
    def run_pagerank(self, node_weights,  damping_factor=0.85, iterations=10):
        ...
        for it in range(iterations):
            ...
            for source, target in self.read_edge_file(self.edge_file):
                #print(self.node_degree[source])
               
                new_scores[target] +=  scores[source]/self.node_degree[source]
            term2=np.multiply(damping_factor,new_scores)
            term1 = (1 - damping_factor) * node_weights
                #new_scores += (1 - damping_factor) * node_weights
            scores=np.add(term2,term1)
